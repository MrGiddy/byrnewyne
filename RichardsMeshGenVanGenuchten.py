...
# from bokeh.models.widgets import Panel, Tabs  ----> changed below to be Bokeh 3.0+ compatible
...
def showMesh(data,z,eta,etaDual,labelSize,titleSize,legendSize,axisTicksSize,lineWidth):
    ...
    p1 = figure(width=600, height=600,tools=['pan,wheel_zoom,box_zoom,reset'],
           title="Mouse over the dots",x_range=(-0.7, 0.7))
    p1.add_tools(hover) # Use add_tools() method to add HoverTool object to figure

    # p1.scatter(x,eta, color="blue", marker='circle',legend='Centroids')
    p1.scatter(x,eta, color="blue", marker='circle',legend_label='Centroids')

    # p1.scatter(x,etaDual, color="fuchsia", marker='cross',legend='CV interface')
    p1.scatter(x,etaDual, color="fuchsia", marker='cross',legend_label='CV interface')
    ...
    for i in range(0,np.size(data.index)):
        ...
        # p1.line([-0.2,0.2], [data['eta'][i],data['eta'][i]], color=c,line_width=lineWidth,label=l)
        p1.line([-0.2,0.2], [data['eta'][i],data['eta'][i]], color=c,line_width=lineWidth,legend_label=l)

    # p1.line([-0.2,0.2], [data['eta'][data['eta'].size-1],data['eta'][data['eta'].size-1]], color="red",line_width=lineWidth,legend='layer')
    p1.line([-0.2,0.2], [data['eta'][data['eta'].size-1],data['eta'][data['eta'].size-1]], color="red",line_width=lineWidth,legend_label='layer')
